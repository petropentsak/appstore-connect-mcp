    ...
    return {
      tools: [
        ...
        },
        {
          name: 'attach_build',
          description:
            'Attach a processed TestFlight build to an App Store version. Single-shot: returns attached | processing | not_found so the caller can retry while the build is still processing.',
          inputSchema: {
            type: 'object',
            properties: {
        ...
        },
        {
          ...
          description:
            'Submit an App Store version for review (iOS reviewSubmissions flow). Optionally sets the release type first.',
          inputSchema: {
            type: 'object',
            properties: {
              appId: { type: 'string', description: 'Numeric app ID or bundle ID' },
        ...
        },
        {
          ...
          description:
            'Release an approved App Store version that is waiting for manual developer release. Resolves the version in PENDING_DEVELOPER_RELEASE when versionString is omitted.',
          inputSchema: {
            type: 'object',
            properties: {
              appId: { type: 'string', description: 'Numeric app ID or bundle ID' },
        ...
        },
        {
          ...
          description:
            'Control the iOS 7-day phased release for a version: get current state, start (ACTIVE), pause, resume, or complete (release to all users at once).',
          inputSchema: {
            type: 'object',
            properties: {
              appId: { type: 'string', description: 'Numeric app ID or bundle ID' },
        ...
        },
        {
          ...
          description:
            'Get available price points for an app in a territory (id + customer price + proceeds). The id feeds update_price_schedule.',
          inputSchema: {
            type: 'object',
            properties: {
              appId: { type: 'string', description: 'Numeric app ID or bundle ID' },
        ...
        },
        {
          ...
          description:
            'Set an app price by creating a new price schedule pinned to a price point in a base territory (modern pricing API).',
          inputSchema: {
            type: 'object',
            properties: {
              appId: { type: 'string', description: 'Numeric app ID or bundle ID' },
        ...
        },
        {
          ...
          description:
            'Upload one screenshot to an app screenshot set (reserve, upload byte ranges to pre-signed URLs, commit with MD5).',
          inputSchema: {
            type: 'object',
            properties: {
...
${details.secondarySubcategoryTwo ? `• Secondary Subcategory 2: ${details.secondarySubcategoryTwo}` : ''}` : `No detailed info found for app ${appId}.`,
        ...
        case 'attach_build': {
          const { appId, versionId, buildId, buildVersionString } = args as {
            appId: string;
            versionId: string;
            buildId?: string;
        ...
        case 'submit_for_review': {
          const { appId, versionId, releaseType } = args as {
            appId: string;
            versionId: string;
        ...
        case 'release_version': {
          const { appId, versionString } = args as { appId: string; versionString?: string };
        ...
        case 'manage_phased_release': {
          const { appId, versionString, action } = args as {
            appId: string;
            versionString?: string;
        ...
        case 'set_beta_whats_new': {
          const { buildId, locale, whatsNew } = args as {
            buildId: string;
            locale?: string;
            whatsNew: string;
          };
        ...
        case 'get_app_price_points': {
          const { appId, territory } = args as { appId: string; territory?: string };
            ...
            content: [
              {
                ...
                text: points.length
                  ? `💵 Price points for app ${appId} (${territory || 'USA'}):\n\n${points
                      .map(
                        (p, i) =>
                          `${i + 1}. ${p.customerPrice ?? 'N/A'} (proceeds: ${p.proceeds ?? 'N/A'})\n   • ID: ${p.id}`,
                      )
                      .join('\n')}`
        ...
        case 'update_price_schedule': {
          const { appId, territory, pricePointId } = args as {
            appId: string;
            territory: string;
            pricePointId: string;
          };
        ...
        case 'set_app_availability': {
          const { appId, territories, availableInNewTerritories } = args as {
            appId: string;
            territories: string[];
        ...
        case 'upload_screenshot': {
          const { screenshotSetId, filePath } = args as {
