...
function formatCapabilities(capabilities) {
    ...
    return capabilities
        .map((capability) => {
        const key = capability.entitlementKey === undefined
        ...
        return {
            tools: [
                ...
                },
                {
                    ...
                    description: "List developer-portal bundle IDs with the capabilities enabled on each and the entitlement key every capability authorises. App records in App Store Connect expose no entitlements — the portal bundle ID is where they live. A capability is Apple's grant; the app must still request the key in its .entitlements file for a build to claim it.",
                    inputSchema: {
                        type: 'object',
                        properties: {
                            identifier: {
                                type: 'string',
                                description: 'Case-insensitive substring filter, e.g. "eu.ecofactor". Matched locally, so it cannot mis-hit the way Apple\'s partial filter[identifier] does.',
                            },
                            platform: {
                                type: 'string',
                ...
                },
                {
                    ...
                    description: 'Get the capabilities enabled on one bundle ID, each mapped to the entitlement key it authorises (e.g. CARPLAY_NAVIGATION -> com.apple.developer.carplay-maps).',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            bundleId: {
                                type: 'string',
                                description: 'Exact bundle identifier (e.g. eu.ecofactor) or portal resource id (e.g. 636AMV3G4A)',
                            },
                        },
                        required: ['bundleId'],
                    },
                ...
                {
                    name: 'list_profiles',
                    description: 'List provisioning profiles with type, state, UUID, expiry and the bundle ID each belongs to.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            bundleId: {
                            ...
                            profileType: {
                                type: 'string',
                                description: 'e.g. IOS_APP_STORE, IOS_APP_DEVELOPMENT, IOS_APP_ADHOC, IOS_APP_INHOUSE',
                            },
                ...
                },
                {
                    ...
                    description: 'Decode a provisioning profile and return the entitlements it authorises — the ground truth for whether a build can sign a given entitlement. Pass profileId, or pass bundleId to take the newest ACTIVE profile of profileType (default IOS_APP_STORE).',
                    inputSchema: {
                        type: 'object',
                        properties: {
...
${details.secondarySubcategoryTwo ? `• Secondary Subcategory 2: ${details.secondarySubcategoryTwo}` : ''}` : `No detailed info found for app ${appId}.`,
                ...
                case 'list_bundle_ids': {
                    const { identifier, platform } = args;
                        ...
                        content: [
                            {
                                ...
                                text: `Found ${bundles.length} bundle ID(s):\n\n${bundles
                                    .map((bundle) => `• ${bundle.identifier} — ${bundle.name} (${bundle.platform}) [${bundle.id}]\n${formatCapabilities(bundle.capabilities)}`)
                                    .join('\n\n')}`,
                            },
                        ],
                    };
                ...
                case 'list_profiles': {
                    const { bundleId, profileType, profileState, limit } = args;
                ...
                case 'get_profile_entitlements': {
                    const { profileId, bundleId, profileType } = args;
